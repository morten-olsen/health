...
**ALWAYS include `.js` file extensions in imports** — TypeScript resolves `.js` to `.ts` at compile time, and the output works as-is at runtime.

- Use `.js` for all TypeScript source imports (both within and across packages)
- Use `.jsx` for files containing JSX (frontend code)
- This applies to both relative imports and workspace package internals

```typescript
// ✅ Good — .js extension for all TS imports
import { createUser } from "./user-service.js";
import type { User } from "./types.js";

// ✅ Good — cross-package imports use package name
import { metricSampleSchema } from "@morten-olsen/health-contracts";

// ✅ Good — frontend JSX files
import { useAuth } from "./auth/auth.jsx";

// ❌ Bad - .ts extension (breaks at runtime without type stripping)
import { createUser } from "./user-service.js";
...
- `.js` extensions work with both `tsc` output and `tsx` dev runner
- TypeScript resolves `.js` imports to `.ts` files during compilation — this is by design
- Consistent across all packages in the monorepo (no special cases)
...
1. External dependencies (node builtins and npm packages)
2. Workspace packages (`@morten-olsen/health-*`)
3. Relative imports
...
```typescript
import { formatUserId } from "./user.utils.js";
import { UserNotFoundError } from "./user.errors.js";
import { userSchema, createUserInputSchema } from "./user.schemas.js";

import type { User, CreateUserInput } from "./user.schemas.js";
...
export type { User, CreateUserInput };
export { userSchema, createUserInputSchema };
export { UserNotFoundError, UserValidationError } from "./user.errors.js";
...
import { createUser, getUserById, UserNotFoundError } from "./user/user.js";
import type { User } from "./user/user.js";

// ❌ Bad - importing from support files directly
import { userSchema } from "./user/user.schemas.js";
import { formatUserId } from "./user/user.utils.js";
```
...
import type { ApiClient } from "../types/api.js";
